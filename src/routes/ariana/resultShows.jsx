import styles from './resultShows.module.css';

const Shows = () => {
    return (
        <article className={styles.resultShows}>
            <h3 className={styles.thisYear}>- 2024 -</h3>
            <table frame="void" rules='none' className={styles.thisYearTable}>
                <tbody>
                <tr>
                        <td>1.12.</td>
                        <td>Triocacib Prague Expo Dog, CZ</td>
                        <td>ANtonín Karban, CZ</td>
                        <td>xx</td>
                    </tr>

                <tr>
                        <td>30.11.</td>
                        <td>Triocacib Prague Expo Dog, CZ</td>
                        <td>rozhodčí</td>
                        <td>xx</td>
                    </tr>
                <tr>
                        <td>29.11.</td>
                        <td>Triocacib Prague Expo Dog, CZ</td>
                        <td>rozhodčí</td>
                        <td>xx</td>
                    </tr>
                <tr>
                        <td>15.9.</td>
                        <td>Šumavská mezinárodní výstava Klatovy, CZ</td>
                        <td>Lenka Frnčová, CZ</td>
                        <td>V2, res.CAC</td>
                    </tr>
                    <tr>
                        <td>11.8.</td>
                        <td>Duo CACIB Sopot, PL</td>
                        <td>Piotr Król, PL</td>
                        <td>V1, CAC</td>
                    </tr>
                    <tr>
                        <td>10.8.</td>
                        <td>Duo CACIB Sopot, PL</td>
                        <td>Mariola Agata Semik, PL</td>
                        <td>V1, CAC</td>
                    </tr>
                    <tr>
                        <td>21.7.</td>
                        <td>Národní výstava Mladá Boleslav, CZ</td>
                        <td>Ladislav Frnčo, CZ</td>
                        <td>VD2</td>
                    </tr>
                    <tr>
                        <td>7.7.</td>
                        <td>Dogfestival Veľká Ida, SK</td>
                        <td>Zuzana Mikolková-Betinská, SK</td>
                        <td>V1, CAC, CACIB, BOS</td>
                    </tr>
                    <tr>
                        <td>6.7.</td>
                        <td>Dogfestival Veľká Ida, SK</td>
                        <td>Antonín Karban, CZ</td>
                        <td>V1, CAC, res.CACIB</td>
                    </tr>
                    <tr>
                        <td>5.7.</td>
                        <td>Dogfestival Veľká Ida, SK</td>
                        <td>Martin Šipkovský, SK</td>
                        <td>V1, CAC</td>
                    </tr>
                    <tr>
                        <td>30.6.</td>
                        <td>Národní Klatovy, CZ</td>
                        <td>Kristina Vaníčková, CZ</td>
                        <td>VD1</td>
                    </tr>
                    <tr>
                        <td>23.6.</td>
                        <td>Intercanis Brno, CZ</td>
                        <td>Hana Chovancová, CZ</td>
                        <td>V2, res.CAC</td>
                    </tr>
                    <tr>
                        <td>9.6.</td>
                        <td>Duo CACIB Bytom , PL</td>
                        <td>Aleksandar Petrovic, RS</td>
                        <td>V2</td>
                    </tr>
                    <tr>
                        <td>8.6.</td>
                        <td>Duo CACIB Bytom , PL</td>
                        <td>Kimmo Mustonen, FI</td>
                        <td>V3</td>
                    </tr>
                    <tr>
                        <td>19.5.</td>
                        <td>Krajská výstava Praha, CZ</td>
                        <td>Ladislav Frnčo, CZ</td>
                        <td>V1, vítěz třídy, Krajský vítěz, 2.místo BIS feny</td>
                    </tr>
                    <tr>
                        <td>4.5.</td>
                        <td>Krajská Florinka Olomouc, CZ</td>
                        <td>Antonín Mudra, CZ</td>
                        <td>V1, vítěz třídy</td>
                    </tr>
                    <tr>
                        <td>21.4.</td>
                        <td>Czech Northern Cup - klubová, Nová Živohošť, CZ</td>
                        <td>Elaine Walker, IE</td>
                        <td>V2, res.CAC</td>
                    </tr>
                    <tr>
                        <td>20.4.</td>
                        <td>Czech Northern Cup - klubová, Nová Živohošť, CZ</td>
                        <td>Andrzej Szutkiewicz, PL</td>
                        <td>V3</td>
                    </tr>
                    <tr>
                        <td>17.3.</td>
                        <td>Národní výstava Špilberka, CZ</td>
                        <td>Miroslav Václavík, CZ</td>
                        <td>VD3</td>
                    </tr>
                    <tr>
                        <td>18.2.</td>
                        <td>Nitradog Pribina cup, SK</td>
                        <td>Rafael Garcia Parrondo, ES</td>
                        <td>V2, res.CAC</td>
                    </tr>
                    <tr>
                        <td>17.2.</td>
                        <td>Nitradog Pribina cup, SK</td>
                        <td>Vija Klučniece, LV</td>
                        <td>V1, CAC, res.CACIB</td>
                    </tr>
                    <tr>
                        <td>21.1.</td>
                        <td>Bratislava WinterDanube CACIB, SK</td>
                        <td>Martin Šipkovský, SK</td>
                        <td>V1, CAC, CACIB</td>
                    </tr>
                    <tr>
                        <td>20.1.</td>
                        <td>Bratislava WinterDanube CACIB, SK</td>
                        <td>Zuzana MIkolková-Betinská, SK</td>
                        <td>V1</td>
                    </tr>
                </tbody>
            </table>

            <h3 className={styles.thisYear}>- 2023 -</h3>
            <table frame="void" rules='none' className={styles.thisYearTable}>
                <tbody>
                    <tr>
                        <td>10.12.</td>
                        <td>Winter Triocacib Prague Expo Dog, CZ</td>
                        <td>Gabriela Richard-Steinbach, DE</td>
                        <td>VD1</td>
                    </tr>
                    <tr>
                        <td>9.12.</td>
                        <td>Winter Triocacib Prague Expo Dog, CZ</td>
                        <td>Rafael Garcia Parrondo, ES</td>
                        <td>V1, CAC</td>
                    </tr>
                    <tr>
                        <td>8.12.</td>
                        <td>Winter Triocacib Prague Expo Dog, CZ</td>
                        <td>Carmen Guisado Navarro, ES</td>
                        <td>VD1</td>
                    </tr>
                    <tr>
                        <td>1.10.</td>
                        <td>Krajská výstava Praha, CZ</td>
                        <td>Olga Dolejšová, CZ</td>
                        <td>V1, vítěz třídy</td>
                    </tr>
                    <tr>
                        <td>23.9.</td>
                        <td>Klubová výstava Nová Živohošť Klub severských psů, CZ</td>
                        <td>Erna Upmeijer, NLD</td>
                        <td>VD2</td>
                    </tr>
                    <tr>
                        <td>10.9.</td>
                        <td>Floracanis Olomouc, CZ</td>
                        <td>Oliver Šimon, CZ</td>
                        <td>V2</td>
                    </tr>
                    <tr>
                        <td>29.7.</td>
                        <td>Oblastní výstava Hubertka Bordovice, CZ</td>
                        <td>Vladimír Piskay, SK</td>
                        <td>V2</td>
                    </tr>
                    <tr>
                        <td>25.6.</td>
                        <td>Intercanis Brno, CZ</td>
                        <td>Zsuszanna Vácsi - Balogh, HU </td>
                        <td>V2</td>
                    </tr>
                    <tr>
                        <td>18.6.</td>
                        <td>Oblastní Náměšťská výstava, CZ</td>
                        <td>Lenka Frnčová, CZ</td>
                        <td>V2</td>
                    </tr>
                    <tr>
                        <td>3.6.</td>
                        <td>Krajská výstava Zbraslav u Brna</td>
                        <td>Petr Řehánek, CZ</td>
                        <td>V2</td>
                    </tr>
                    <tr>
                        <td>7.5.</td>
                        <td>Krajská Florinka Olomouc, CZ</td>
                        <td>Petr Řehánek, CZ</td>
                        <td>VN2</td>
                    </tr>
                    <tr>
                        <td>30.4.</td>
                        <td>Krajská Hornoslezská výstava Dolní Benešov, CZ</td>
                        <td>Lenka Frnčová, CZ</td>
                        <td>VN2</td>
                    </tr>
                    <tr>
                        <td>16.4.</td>
                        <td>Mezinárodní výstava České Budějovice, CZ</td>
                        <td>Otakar Vondrouš, CZ</td>
                        <td>VN2</td>
                    </tr>
                    <tr>
                        <td>15.4.</td>
                        <td>Klubová výstava Chržín Samojed klub, CZ</td>
                        <td>Monika Milian, PL</td>
                        <td>VN2</td>
                    </tr>
                    <tr>
                        <td>26.3.</td>
                        <td>Oblastní výstava Kelč, CZ</td>
                        <td>Alan Matthews, IE</td>
                        <td>VN1</td>
                    </tr>
                </tbody>
            </table>

        </article>
    )
}
export default Shows;